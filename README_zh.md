...
>
> **G2 分數於 2026-03-21 更新：**驗證腳本已修正，現在接受將 SKILL.md 放在子目錄中（例如 `pomodoro/SKILL.md` 而非 `./SKILL.md`）。原始驗證器只檢查工作區根目錄，不公平地懲罰了 10 個將檔案組織到專案子目錄的模型 — 這是合理的慣例，而非寫程式錯誤。最大改善：**qwen3.5-122b**（10→27，+17）、**gpt-oss-20b**（7→23，+16）、**GLM-5**（8→24，+16）、**qwen3.5-397b**（17→26，+9）。
...
2. **qwen3.5-27b 躍升至第二名（51/60）** — 最強通用模型，Dense 27B 架構
3. **驗證修正大幅改變排名** — 原始驗證器因檔案放置在子目錄而扣分，修正後 10 個模型的 G2 分數提升了 +4 至 +17 分（詳見排行榜下方說明）
